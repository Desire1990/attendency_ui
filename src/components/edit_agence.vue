...
export default {
	...
	methods: {
		...
		changerService(){
			...
			axios.put(this.$store.state.url+`/agence/${this.service.id}/`, this.new_service, headers)
			.then((response) => {
				let agences = this.$store.state.agences;
				for(var i = 0; i < agences.length; i++){
					let x = agences[i];
					if(x.id==this.service.id){
						this.$store.state.agences[i] = response.data;
